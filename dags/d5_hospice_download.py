...
S3_PREFIX_RAW = "raw/hospice"
S3_PREFIX_BRONZE = "bronze/hospice"
...
with DAG(
    ...
    doc_md="""
    # Hospice CSV DAG
    Télécharge le CSV Hospice - General Information et le stocke sur S3 (raw + bronze Parquet).
    """,
    ...
    def upload_csv_and_parquet(**context):
        ...
        # --- Upload raw CSV ---
        s3_key_versioned = f"{S3_PREFIX_RAW}/Hospice_{timestamp}.csv"
        s3_key_latest = f"{S3_PREFIX_RAW}/Hospice_LATEST.csv"
        ...
        parquet_local_path = "/tmp/Hospice_Info.parquet"
        df.to_parquet(parquet_local_path, index=False)
        s3_key_parquet = f"{S3_PREFIX_BRONZE}/Hospice_LATEST.parquet"
