...
CSV_URL = "https://data.cms.gov/provider-data/sites/default/files/resources/66f260a1b66c08187c24fee8d189943b_1767384363/NH_ProviderInfo_Dec2025.csv"
S3_BUCKET = "ai-factory-bckt"
S3_PREFIX = "raw/provider_information"
...
# -------------------------------------------------------------------
with DAG(
    dag_id="provider_informations_download",
    default_args=default_args,
    schedule=None,
    catchup=False,
    doc_md="""
    # Provider Information CSV DAG
    Télécharge le CSV Provider Information (Nursing Home) et le stocke sur S3.
    """,
    tags=['provider', 'csv', 's3'],
) as dag:
    ...
    def download_csv(**context):
        local_path = "/tmp/NH_ProviderInfo_Dec2025.csv"
    ...
    def upload_csv_to_s3(**context):
        ...
        s3_key_versioned = f"{S3_PREFIX}/NH_ProviderInfo_{timestamp}.csv"
        s3_key_latest = f"{S3_PREFIX}/NH_ProviderInfo_LATEST.csv"
    ...
    def summarize_run(**context):
        ...
        logger.info("="*70)
        logger.info("📊 PROVIDER INFORMATION DAG - SUMMARY")
        logger.info("="*70)
    ...
    # -------------------
    # TASK 4 : Trigger DAG d4
    # -------------------
    trigger_longterm_care_hospital = TriggerDagRunOperator(
        task_id='trigger_longterm_care_hospital',
        trigger_dag_id='longterm_care_hospital_download', # DAG ID
    ...
    # DAG DEPENDENCIES
    # -------------------------------------------------------------------
    download_task >> upload_task >> summary_task >> trigger_longterm_care_hospital