...
S3_PREFIX_RAW = "raw/provider_information"
S3_PREFIX_BRONZE = "bronze/provider_information"
...
# -------------------------------------------------------------------
with DAG(
    dag_id="provider_information_download",
    default_args=default_args,
    schedule=None,
    catchup=False,
    doc_md="""
    # Provider Information CSV DAG
    Télécharge le CSV Provider Information (Nursing Home) et le stocke sur S3 (raw + bronze Parquet).
    """,
    ...
    def upload_csv_and_parquet(**context):
        ...
        # --- Upload raw CSV ---
        s3_key_versioned = f"{S3_PREFIX_RAW}/NH_ProviderInfo_{timestamp}.csv"
        s3_key_latest = f"{S3_PREFIX_RAW}/NH_ProviderInfo_LATEST.csv"
        ...
        parquet_local_path = "/tmp/NH_ProviderInfo.parquet"
        df.to_parquet(parquet_local_path, index=False)
        s3_key_parquet = f"{S3_PREFIX_BRONZE}/NH_ProviderInfo_LATEST.parquet"
